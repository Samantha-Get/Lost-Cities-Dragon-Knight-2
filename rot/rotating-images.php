...
if ( ! class_exists( 'iThemesRotatingImages' ) ) {
	class iThemesRotatingImages {
		...
		function __construct() {
			...
			add_action( 'admin_init', array( &$this,'ithemes_rotatingimages_add_privacy_policy_content' ) );
		...
		function ithemes_rotatingimages_add_privacy_policy_content() {
    		if ( ! function_exists( 'wp_add_privacy_policy_content' ) ) {
        		return;
    		}
 
   			 $content = __( 'This plugin does not record or store any personal data.', 'it-l10n-rotating-images'  );
		...
		function addAdminScripts() {
			...
			wp_enqueue_script( $this->_var . '-it-tooltip', $this->_pluginURL . '/js/tooltip.js', array('jquery') );
			wp_print_scripts( $this->_var . '-it-tooltip' );
			wp_enqueue_script( $this->_var . '-toolkit', $this->_pluginURL . '/js/javascript-toolbox-toolkit.js', array('jquery') );
			wp_print_scripts( $this->_var . '-toolkit' );
			if ( isset( $_GET['group_id'] ) ) { // Only show when viewing a group to avoid errors.
				wp_enqueue_script( $this->_var . '-rotating-images', $this->_pluginURL . '/js/admin-rotating-images.js', array( 'wp-color-picker' ) );
				wp_print_scripts( $this->_var . '-rotating-images');
				wp_enqueue_script( $this->_var . '-reorder-js', $this->_pluginURL . '/js/tablednd.js', array('jquery') );
				wp_print_scripts( $this->_var . '-reorder-js' );
			}
			wp_enqueue_script( $this->_var . '-adminjs', $this->_pluginURL . '/js/admin.js', array('jquery') );
				...
				<form enctype="multipart/form-data" method="post" action="<?php echo $this->_selfLink; ?>&group_id=<?php echo $this->_groupID; ?>">
					<table class="form-table">
								...
								<tr>
									...
									<td>
										<?php $this->_addTextBox( 'overlay_header_color', array( 'size' => '7' ) ); ?>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>
										<?php $this->_addTextBox( 'overlay_subheader_color', array( 'size' => '7' ) ); ?>
									</td>
								</tr>
								<tr>
