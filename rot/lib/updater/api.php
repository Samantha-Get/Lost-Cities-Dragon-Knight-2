...
class Ithemes_Updater_API {
	...
	public static function get_package_changelog( $package, $cur_version = false ) {
		...
		if ( ! isset( $response['body'] ) ) {
			return new WP_Error( 'ithemes-updater-changelog-bad-wp-remote-get-response', __( 'Unrecognized response from <code>wp_remote_get</code>.', 'it-l10n-rotating-images' ) );
		}
		...
		if ( '{' === substr( $body, 0, 1 ) ) {
			...
			if ( is_array( $error ) && isset( $error['error'] ) && is_array( $error['error'] ) && isset( $error['error']['type'] ) && isset( $error['error']['message'] ) ) {
				...
				return new WP_Error( 'ithemes-updater-changelog-bad-json-decode-result', __( 'Unrecognized response from iThemes API server.', 'it-l10n-rotating-images' ) );
			}
		}
