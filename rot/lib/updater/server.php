	...
	public static function request( $action, $query = array(), $data = array() ) {
		...
		if ( false !== ( $timeout_start = get_site_option( 'ithemes-updater-server-timed-out' ) ) ) {
			...
			if ( $time_remaining < 0 ) {
				...
				return new WP_Error( 'ithemes-updater-timed-out-server', sprintf( _n( 'The server could not be contacted. Requests are delayed for %d minute to allow the server to recover.', 'The server could not be contacted. Requests are delayed for %d minutes to allow the server to recover.', $minutes_remaining, 'it-l10n-rotating-images' ), $minutes_remaining ) );
			}
		}
		...
		if ( is_wp_error( $response ) ) {
			if ( 'connect() timed out!' == $response->get_error_message() ) {
				...
				return new WP_Error( 'http_request_failed', __( 'The server was unable to be contacted.', 'it-l10n-rotating-images' ) );
			}
		...
		if ( ! empty( $body['error'] ) ) {
			return new WP_Error( $body['error']['type'], sprintf( __( 'An error occurred when communicating with the iThemes update server: %s (%s)', 'it-l10n-rotating-images' ), $body['error']['message'], $body['error']['code'] ) );
		}
