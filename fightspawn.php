...
function fightmonster() { // One big long function that determines the outcome of the fight.
...
//    if ($userrow["currentaction"] != "Fighting") { display("Cheat attempt detected.<br /><br />Get a life, loser.", "Error"); }
    ...
    if ($userrow["currentfight"] == 2) {
        
//        if ($userrow["latitude"] < 0) { $userrow["latitude"] *= -1; } // Equalize negatives.
//        if ($userrow["longitude"] < 0) { $userrow["longitude"] *= -1; } // Ditto.
//        $maxlevel = floor(max($userrow["latitude"]+5, $userrow["longitude"]+5) / 5); // One mlevel per ten [was five] spaces.
//        if ($maxlevel < 1) { $maxlevel = 1; }
//        $minlevel = $maxlevel - 2;
//        if ($minlevel < 1) { $minlevel = 1; }
        
        // Pick a fixed monster.
    ...
    if (isset($_POST["run"])) {
        ...
        $chancetorun = rand(6,10) + ceil(sqrt($userrow["dexterity"]));
        if ($chancetorun > (rand(3,7) + ceil(sqrt($monsterrow["maxdam"])))) { $chancetorun = 1; } else { $chancetorun = 0; }
        ...
        if ($chancetorun == 0) { 
            ...
            if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.
                ...
                $toblock = ceil(rand($userrow["defensepower"]*.96,$userrow["defensepower"])/3);
                $tododge = rand(1,150);
    ...
    // Do fight stuff.
        ...
        // Your turn.
        $pagearray["yourturn"] = "";
        $tohit = ceil(rand($userrow["attackpower"]*.95,$userrow["attackpower"])/4);
        ...
        if ($tododge <= sqrt($monsterrow["armor"])) { 
            ...
            if ($userrow["currentuberdamage"] != 0) {
                $monsterdamage += ceil($monsterdamage * ($userrow["currentuberdamage"]/85));
            }
        }
        ...
        if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.
            $tohit = ceil(rand($monsterrow["maxdam"]*.6,$monsterrow["maxdam"]));
            if ($userrow["difficulty"] == 2) { $tohit = ceil($tohit * $controlrow["diff2mod"]); }
            if ($userrow["difficulty"] == 3) { $tohit = ceil($tohit * $controlrow["diff3mod"]); }
            if ($userrow["difficulty"] == 4) { $tohit = ceil($tohit * $controlrow["diff4mod"]); }
            if ($userrow["difficulty"] == 5) { $tohit = ceil($tohit * $controlrow["diff5mod"]); }
            ...
            if ($userrow["difficulty"] == 10) { $tohit = ceil($tohit * $controlrow["diff10mod"]); }
            if ($userrow["difficulty"] == 11) { $tohit = ceil($tohit * $controlrow["diff11mod"]); }
            $toblock = ceil(rand($userrow["defensepower"]*.95,$userrow["defensepower"])/5);
            $tododge = rand(1,150);
            if ($tododge <= sqrt($userrow["dexterity"])) {
                $tohit = 0; $pagearray["monsterturn"] .= "You dodge the monster's attack. No damage has been scored.<br />";
                $persondamage = 0;
    ...
    // Do spell stuff.
        ...
        if ($pickedspell == 0) { display("<center><h3 class="title">Spell not Selected</h3></center><center><blockquote><blockquote><br /><br />You must select a spell first. Please go back and try again.</blockquote></blockquote></center><br /><br />", "Error"); die(); }
        ...
        if ($spell != true) { display("<center><h3 class="title">Spell not Known by You</h3></center><center><blockquote><blockquote><br /><br />You have not yet learned this spell. Please go back and try again.</blockquote></blockquote></center><br /><br />", "Error"); die(); }
        if ($userrow["currentmp"] < $newspellrow["mp"]) { display("<center><h3 class="title">More Magic Points Needed</h3></center><center><blockquote><blockquote><br /><br />You do not have enough Magic Points to cast this spell. Please go back and try again.</blockquote></blockquote></center><br /><br />", "Error"); die(); }
        ...
        if ($newspellrow["type"] == 1) { // Heal spell.
            ...
            $pagearray["yourturn"] = "<center><h3 class="title">Casting Heal Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell, and gained ".$newspellrow["attribute"]." Hit Points.</blockquote></blockquote></center><br /><br />";
            ...
            if ($userrow["currentmonsterimmune"] == 0) {
                ...
                $userrow["currentmonsterhp"] -= $monsterdamage;
                $pagearray["yourturn"] = "<center><h3 class="title">Casting Hurt Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell for $monsterdamage damage.</blockquote></blockquote></center><br /><br />";
            } else {
                $pagearray["yourturn"] = "<center><h3 class="title">Casting Hurt Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell, but the monster is immune to it.</blockquote></blockquote></center><br /><br />";
            }
            $userrow["currentmp"] -= $newspellrow["mp"];
            ...
            if ($userrow["currentmonsterimmune"] != 2) {
                ...
                $pagearray["yourturn"] = "<center><h3 class="title">Casting Sleep Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell. The monster is asleep.</blockquote></blockquote></center><br /><br />";
            } else {
                $pagearray["yourturn"] = "<center><h3 class="title">Casting Sleep Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell, but the monster is immune to it.</blockquote></blockquote></center><br /><br />";
            }
            $userrow["currentmp"] -= $newspellrow["mp"];
        } elseif ($newspellrow["type"] == 4) { // +Damage spell.
            $userrow["currentuberdamage"] = $newspellrow["attribute"];
            $userrow["currentmp"] -= $newspellrow["mp"];
            $pagearray["yourturn"] = "<center><h3 class="title">Casting Damage Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell, and will gain ".$newspellrow["attribute"]."% damage until the end of this fight.</blockquote></blockquote></center><br /><br />";
        } elseif ($newspellrow["type"] == 5) { // +Defense spell.
            $userrow["currentuberdefense"] = $newspellrow["attribute"];
            $userrow["currentmp"] -= $newspellrow["mp"];
            $pagearray["yourturn"] = "<center><h3 class="title">Casting Defense Spell</h3></center><center><blockquote><blockquote><br /><br />You have cast the ".$newspellrow["name"]." spell, and will gain ".$newspellrow["attribute"]."% defense until the end of this fight.</blockquote></blockquote></center><br /><br />";            
        }
...
END;
    ...
    $pagearray["command"] = "<center><h3 class="title">Casting Defense Spell</h3></center><center><blockquote><blockquote><br /><br />You have died.<br /><br />As a consequence, you've lost half of your gold. However, you have been given back a portion of your hit points to continue your journey.<br /><br />You may now continue back to the<br /><br /<a href=\"index.php\" class=\"myButton2\">Town Square</a><br /><br />and we hope you fair better next time.<br />On your fifth death will be final.<br />No Magic in this land can bring you back from a Fifth and Final Death.<br />You Soul will then rest forever in the Town Graveyard.</blockquote></blockquote></center><br /><br />";
}
...
function victory() {
    ...
    $gold = rand((($monsterrow["maxgold"]/5)*6),$monsterrow["maxgold"]);
    if ($gold < 1) { $gold = 1; }
    ...
    if ($userrow["experience"] + $exp < 101202303) { $newexp = $userrow["experience"] + $exp; $warnexp = ""; } else { $newexp = $userrow["experience"]; $exp = 0; $warnexp = "You have maxed out your experience points."; }
    if ($userrow["gold"] + $gold < 101202303) { $newgold = $userrow["gold"] + $gold; $warngold = ""; } else { $newgold = $userrow["gold"]; $gold = 0; $warngold = "You have maxed out your experience points."; }
    ...
    if ($userrow["level"] < 300) {
        if ($newexp >= $levelrow[$userrow["charclass"]."_exp"]) {
            ...
            $page = "<center><h3 class='title'>Victory</h3></center><center><blockquote><blockquote>Congratulations. You have defeated the ".$monsterrow["name"].".<br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold <br /><br />You have gained a level!<br /><br />You gain ".$levelrow[$userrow["charclass"]."_hp"]." hit points.<br />You gain ".$levelrow[$userrow["charclass"]."_mp"]." magic points.<br />You gain ".$levelrow[$userrow["charclass"]."_tp"]." travel points.<br />You gain ".$levelrow[$userrow["charclass"]."_strength"]." strength.<br />You gain ".$levelrow[$userrow["charclass"]."_dexterity"]." dexterity.<br />$spelltext<br /><font color=\"#804000\">You can now continue [root/fightspawn.php 455]</font><br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a></blockquote></blockquote></center><br /><br />";
            ...
            $newspell = "";
            $page = "<center><h3 class='title'>Victory</h3></center><center><blockquote><blockquote>Congratulations. You have defeated the ".$monsterrow["name"].".<br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold [root/fightspawn.php 467]</blockquote></blockquote></center><br /><br />";
            ...
            if (rand(1,1) == 1) {
                ...
                $page .= "This monster has dropped an item. <a href=\"index.php?do=drop\">Click here</a> to reveal and equip the item, or you may also move on and continue <a href=\"index.php\">exploring</a>  [root/fightspawn.php 473].";
            } else { 
                $dropcode = "";
                $page .= "<font color=\"#804000\">You can now continue [fightspawn.php 477]</font><br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a><br /><br />";
            }
...
function drop() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($slot == 0) { display("<center><h3 class='title'>Select Inventory Slot</h3></center><center><blockquote><blockquote>Please go back and select an inventory slot to continue.</blockquote></blockquote></center><br /><br />","Error"); }
        ...
        $page = "<center><h3 class='title'>Equipped Item</h3></center><center><blockquote><blockquote>The item has been equipped. <font color=\"#804000\">You can now continue [fightspawn.php 552]</font><br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a></blockquote></blockquote></center><br /><br />";
    ...
    $page = "The monster dropped the following item: ".$droprow["name"]."<br /><br />";
    ...
    $page .= "<form action=\"index.php?do=drop\" method=\"post\"><select name=\"slot\"><option value=\"0\">Choose One</option>
	...
	<option value=\"8\">Slot 8: ".$userrow["slot8name"]."</option>
	</select> <input type=\"submit\" name=\"submit\" value=\"Submit\" /></form>";
    $page .= "You may also choose to just continue and give up this item.<br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a><br /><br />";
...
function dead() {
    $page = "<center><h3 class='title'>Death</h3></center><center><blockquote><blockquote>You have died.<br /><br />As a consequence, you've lost half of your gold. However, you have been given back a portion of your hit points to continue your journey.<br /><br />You may now continue back to <a href=\"index.php\">Town Square</a>, and we hope you fair better next time. Remember after your fifth death, your death will be ever lasting. You body will find its forever resting spot in the town graveyard.</blockquote></blockquote></center><br /><br />";
}
