<?php // moderator.php :: primary moderatoristration script.
...
if ($userrow["authlevel"] != 3) { die("You must have moderator privileges to use the control panel."); }
...
function main() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            $query = doquery("UPDATE {{table}} SET gamename='$gamename',gamesize='$gamesize',forumtype='$forumtype',forumaddress='$forumaddress',compression='$compression',class1name='$class1name',class2name='$class2name',class3name='$class3name',diff1name='$diff1name',diff2name='$diff2name',diff3name='$diff3name',diff2mod='$diff2mod',diff3mod='$diff3mod',gameopen='$gameopen',verifyemail='$verifyemail',gameurl='$gameurl',moderatoremail='$moderatoremail',shownews='$shownews',showonline='$showonline',showbabble='$showbabble' WHERE id='1' LIMIT 1", "control");
            moderatordisplay("Settings updated.","Main Settings");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Main Settings");
        }
    }
...
<form action="moderator.php?do=main" method="post">
<table width="90%">
...
<tr><td width="20%">moderator Email:</td><td><input type="text" name="moderatoremail" size="30" maxlength="100" value="{{moderatoremail}}" /><br /><span class="small">Please specify your email address. This gets used when the game has to send an email to users.</span></td></tr>
...
END;
    ...
    moderatordisplay($page, "Main Settings");
...
function additem() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("New Item Created.","Edit Items");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Create New Items");
        }        
...
$page = <<<END
<b><u>Create New Item</u></b><br /><br />
<form action="moderator.php?do=additem" method="post">
<table width="90%">
...
<option value="1" {{type1select}}>Weapon</option>
<option value="2" {{type2select}}>Armor</option>
<option value="3" {{type3select}}>Shield</option>
</select></td></tr>

<tr><td width="20%">Cost:</td><td><input type="text" name="buycost" size="5" maxlength="10" value="" /> gold</td></tr>
<tr><td width="20%">Attribute:</td><td><input type="text" name="attribute" size="5" maxlength="10" value="" /><br /><span class="small">How much the item adds to total attackpower (weapons/rings) or defensepower (armor/shields/pets).</span></td></tr>
<tr><td width="20%">Special:</td><td><input type="text" name="special" size="30" maxlength="50" value="" /><br /><span class="small">Should be either a special code or <span class="highlight">X</span> to disable. Edit this field very carefully because mistakes to formatting or field names can create problems in the game.</span></td></tr>
</table>
<input type="submit" name="submit" value="Submit" /> <input type="reset" name="reset" value="Reset" />
</form>
...
END;
    ...
    moderatordisplay($page, "Create new Item");
...
function items() {
    ...
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=edititem:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=edititem:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No items found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Items");
...
function edititem($id) {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Item updated.","Edit Items");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Items");
        }        
...
<form action="moderator.php?do=edititem:$id" method="post">
<table width="90%">
...
function delitem() {
	...
	while ($itemrow = mysql_fetch_array($itemquery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delitems\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$itemrow["id"]."\"> <b>".$itemrow["id"]."</b>: <b>".$itemrow["name"]."</b></p></form></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delitems\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$itemrow["id"]."\"> <b>".$itemrow["id"]."</b>: <b>".$itemrow["name"]."</b></p></form></div>\n"; $bg = 1; } 
	}
	moderatordisplay($page, "Del Items");
} 
...
function adddrop() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            $query = doquery("INSERT INTO {{table}} SET name='$name',mlevel='$mlevel',type=1, attribute1='$attribute1',attribute2='$attribute2'", "drops");
            moderatordisplay("Drop Item created.","Add Drops");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Add Drops");
        }        
...
<form action="moderator.php?do=adddrop" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>Autogenerated</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="" /></td></tr>
...
END;
   ...
   $page = parsetemplate($page, $row);
    moderatordisplay($page, "Add Drops");
...
function drops() {
    ...
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=editdrop:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=editdrop:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No items found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Drops");
...
function editdrop($id) {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Item updated.","Edit Drops");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Drops");
        }        
...
<form action="moderator.php?do=editdrop:$id" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>{{id}}</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="{{name}}" /></td></tr>
...
function deldrop() {
	...
	while ($userrow = mysql_fetch_array($userquery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=deldrops\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$userrow["id"]."\"> <b>".$userrow["id"]."</b>: <b>".$userrow["name"]."</b></p></form></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=deldrops\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$userrow["id"]."\"> <b>".$userrow["id"]."</b>: <b>".$userrow["name"]."</b></p></form></div>\n"; $bg = 1; } 
	}
	moderatordisplay($page, "Del Drops");
}
...
function addtown() {
    ...
    if (isset($_POST["submit"])) {
		...
		$query = doquery("INSERT INTO {{table}} SET id='',name='$name',latitude='$latitude',longitude='$longitude',innprice='$innprice',mapprice='$mapprice',travelpoints='$travelpoints',itemslist='$itemslist'", "towns");
            moderatordisplay("Town created.","Edit Towns");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Create Towns");
        }        
...
<form action="moderator.php?do=addtown" method="post">
Type your post below and then click Submit to add it.<br />

<table width="90%">
<tr><td width="20%">ID:</td><td>Autogenerated</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="" /></td></tr>
<tr><td width="20%">Latitude:</td><td><input type="text" name="latitude" size="5" maxlength="10" value="" /><br /><span class="small">Positive or negative integer.</span></td></tr>
<tr><td width="20%">Longitude:</td><td><input type="text" name="longitude" size="5" maxlength="10" value="" /><br /><span class="small">Positive or negative integer.</span></td></tr>
<tr><td width="20%">Inn Price:</td><td><input type="text" name="innprice" size="5" maxlength="10" value="" /> gold</td></tr>
<tr><td width="20%">Map Price:</td><td><input type="text" name="mapprice" size="5" maxlength="10" value="" /> gold<br /><span class="small">How much it costs to buy the map to this town.</span></td></tr>
<tr><td width="20%">Travel Points:</td><td><input type="text" name="travelpoints" size="5" maxlength="10" value="" /><br /><span class="small">How many TP are consumed when travelling to this town.</span></td></tr>
<tr><td width="20%">Items List:</td><td><input type="text" name="itemslist" size="30" maxlength="200" value="" /><br /><span class="small">Comma-separated list of item ID numbers available for purchase at this town. (Example: <span class="highlight">1,2,3,6,9,10,13,20</span>)</span></td></tr>
</table>
<input type="submit" name="submit" value="Submit" /> <input type="reset" name="reset" value="Reset" />
</form>
END;
...
//    $page = parsetemplate($page, $row);
    moderatordisplay($page, "Create New Town");
...
function towns() {
    ...
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=edittown:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=edittown:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No towns found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Towns");
...
function edittown($id) {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Town updated.","Edit Towns");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Towns");
        }        
...
<form action="moderator.php?do=edittown:$id" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>{{id}}</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="{{name}}" /></td></tr>
...
function deltown() {
	...
	while ($townrow = mysql_fetch_array($townquery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=deltowns\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$townrow["id"]."\"> <b>".$townrow["id"]."</b>: <b>".$townrow["name"]."</b></p></form></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=deltowns\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$townrow["id"]."\"> <b>".$townrow["id"]."</b>: <b>".$townrow["name"]."</b></p></form></div>\n"; $bg = 1; } 
	}
	moderatordisplay($page, "Del Towns");
}
...
function monsters() {
    ...
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=editmonster:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=editmonster:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No towns found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Monster");
...
function addmonster() {
    ...
    if (isset($_POST["submit"])) {
		...
		$query = doquery("INSERT INTO {{table}} SET id='',name='$name',maxhp='$maxhp',maxdam='$maxdam',armor='$armor',level='$level',maxexp='$maxexp',maxgold='$maxgold',immune='$immune' ", "monsters");
            moderatordisplay("Monster Created.","Create New Monster");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Create new monster");
        }        
...
<form action="moderator.php?do=addmonster" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>Autogenerated</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="" /></td></tr>
<tr><td width="20%">Max Hit Points:</td><td><input type="text" name="maxhp" size="5" maxlength="10" value="" /></td></tr>
<tr><td width="20%">Max Damage:</td><td><input type="text" name="maxdam" size="5" maxlength="10" value="" /><br /><span class="small">Compares to player's attackpower.</span></td></tr>
<tr><td width="20%">Armor:</td><td><input type="text" name="armor" size="5" maxlength="10" value="" /><br /><span class="small">Compares to player's defensepower.</span></td></tr>
<tr><td width="20%">Monster Level:</td><td><input type="text" name="level" size="5" maxlength="10" value="" /><br /><span class="small">Determines spawn location and item drops.</span></td></tr>
<tr><td width="20%">Max Experience:</td><td><input type="text" name="maxexp" size="5" maxlength="10" value="" /><br /><span class="small">Max experience gained from defeating monster.</span></td></tr>
<tr><td width="20%">Max Gold:</td><td><input type="text" name="maxgold" size="5" maxlength="10" value="" /><br /><span class="small">Max gold gained from defeating monster.</span></td></tr>
...
END;
    ...
    $page = parsetemplate($page, $row);
    moderatordisplay($page, "Add New Monster");
...
function editmonster($id) {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Monster updated.","Edit monsters");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit monsters");
        }        
...
<form action="moderator.php?do=editmonster:$id" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>{{id}}</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="{{name}}" /></td></tr>
...
END;
    ...
    $page = parsetemplate($page, $row);
    moderatordisplay($page, "Edit Monsters");
...
function addspell() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Spell Created.","Add New Spell");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Add New Spell");
        }        
...
<form action="moderator.php?do=addspell" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>Autogenerated</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="" /></td></tr>
...
END;
    ...
    $page = parsetemplate($page, $row);
    moderatordisplay($page, "Add New Spell");
...
function spells() {
    ...
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=editspell:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=editspell:".$row["id"]."\">".$row["name"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No spells found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Spells");
...
function editspell($id) {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("Spell updated.","Edit Spells");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Spells");
        }        
...
<form action="moderator.php?do=editspell:$id" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>{{id}}</td></tr>
<tr><td width="20%">Name:</td><td><input type="text" name="name" size="30" maxlength="30" value="{{name}}" /></td></tr>
...
function delspell() {
	...
	while ($spellrow = mysql_fetch_array($spellquery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delspells\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$spellrow["id"]."\"> <b>".$spellrow["id"]."</b>: <b>".$spellrow["name"]."</b></p></form></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delspells\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$spellrow["id"]."\"> <b>".$spellrow["id"]."</b>: <b>".$spellrow["name"]."</b></p></form></div>\n"; $bg = 1; } 
	}
	moderatordisplay($page, "Del Spells");
...
<form action="moderator.php?do=editlevel" method="post">
<select name="level">
$options
</select> 
...
END;
    ...
    moderatordisplay($page, "Edit Levels");
    
}

function editlevel() {

    if (!isset($_POST["level"])) { moderatordisplay("No level to edit.", "Edit Levels"); die(); }
    $id = $_POST["level"];
...
END;
            ...
            moderatordisplay("Level updated.","Edit Levels");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Spells");
        }        
...
function users() {
    ...
    $count = 1;
    while ($row = mysql_fetch_array($query)) {
        if ($count == 1) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">".$row["id"]."</td><td style=\"background-color: #eeeeee;\"><a href=\"moderator.php?do=edituser:".$row["id"]."\">".$row["username"]."</a></td></tr>\n"; $count = 2; }
        else { $page .= "<tr><td width=\"8%\" style=\"background-color: #ffffff;\">".$row["id"]."</td><td style=\"background-color: #ffffff;\"><a href=\"moderator.php?do=edituser:".$row["id"]."\">".$row["username"]."</a></td></tr>\n"; $count = 1; }
    }
    if (mysql_num_rows($query) == 0) { $page .= "<tr><td width=\"8%\" style=\"background-color: #eeeeee;\">No spells found.</td></tr>\n"; }
    $page .= "</table>";
    moderatordisplay($page, "Edit Users");
...
END;
            ...
            moderatordisplay("User updated.","Edit Users");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Users");
        }        
...
<form action="moderator.php?do=edituser:$id" method="post">
<table width="90%">
<tr><td width="20%">ID:</td><td>{{id}}</td></tr>
...
<tr><td width="20%">Auth Level:</td><td><select name="authlevel"><option value="0" {{auth0select}}>User</option><option value="1" {{auth1select}}>moderator</option><option value="2" {{auth3select}}>Blocked</option><option value="3" {{auth3select}}>Moderator</option></select><br /><span class="small">Set to "Blocked" to temporarily (or permanently) ban a user.</span></td></tr>
...
function deluser() {
	...
	while ($userrow = mysql_fetch_array($userquery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delusers\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$userrow["id"]."\"> <b>".$userrow["id"]."</b>: <b>".$userrow["charname"]."</b></p></form></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=delusers\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$userrow["id"]."\"> <b>".$userrow["id"]."</b>: <b>".$userrow["charname"]."</b></p></form></div>\n"; $bg = 1; } 
	}
	moderatordisplay($page, "Del Users");
...
function addnews() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($errors == 0) { 
            ...
            moderatordisplay("News post added.","Add News");
        } else {
            moderatordisplay("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Add News");
        }        
...
<form action="moderator.php?do=news" method="post">
Type your post below and then click Submit to add it.<br />
...
END;
    ...
    moderatordisplay($page, "Add News");
...
function resetbabble() {
	...
	$page = "<b><u>Reset Babblebox v2.0</b></u><br />© 2006 Greg Smith (xanderman)<br /><br />Click the Delete button next to the appropriate entry to remove that entry from the database, or click the Clear button to erase all data.";
	$babblequery = doquery("SELECT * FROM {{table}} ORDER BY id DESC LIMIT 100", "babble");
	while ($babblerow = mysql_fetch_array($babblequery)) {
		if ($bg == 1) { $page .= "<div style=\"width:98%; background-color:#eeeeee; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=resetbabble\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$babblerow["id"]."\"></form><b>".$babblerow["author"].":</b> ".$babblerow["babble"]."</p></div>\n"; $bg = 2; }
		else { $page .= "<div style=\"width:98%; background-color:#ffffff; font-family: tahoma; font-size: 8pt; line-height: 1.4em; color: #0A3549;\"><form action=\"moderator.php?do=resetbabble\" method=\"post\"><p><input type=\"submit\" value=\"Delete\" name=\"delete\"> <input type=\"hidden\" name=\"id\" value=\"".$babblerow["id"]."\"></form><b>".$babblerow["author"].":</b> ".stripslashes($babblerow["babble"])."</p></div>\n"; $bg = 1; } 
	}
	$page .= "<br /><form action=\"moderator.php?do=resetbabble\" method=\"post\"><input type=\"submit\" value=\"Clear Babblebox\" name=\"clear\"></form>";
	moderatordisplay($page, "Reset Babblebox");
}
