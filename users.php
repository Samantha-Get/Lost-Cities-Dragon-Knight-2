...
if (preg_match("/[^A-z]/", $username)==1) { $errors++; $errorlist .= "Username must be Letters only, Upper or Lower Case or a mixture of both. Examples:  SirMichael, SirJohn, SirJames. This is referred to as your Sign-On Name or Real Name.<br />"; } 
        ...
        if (preg_match("/[^A-z_]/", $charname)==1) { $errors++; $errorlist .= "Username must be Letters only, Upper or Lower Case or a mixture of both or a _ . <br />Examples: Sir_Michael, Sir_John, Sir_James. This is referred to as your in-game Character Name."; } 
        ...
        if ($email1 == "" || $email2 == "") { $errors++; $errorlist .= "Email fields are required.<br />A Account Verification Email will be sent to this address.<br />"; }
        ...
        if (preg_match("/[^A-z0-9_\-]/", $password1)==1) { $errors++; $errorlist .= "Password must be alphanumeric. Examples: Av1_3Nd-D or 12De8-8deT<br />"; } 
        ...
        if ($errors == 0) {
            ...
            if ($controlrow["verifyemail"] == 1) {
                ...
                $page = "Your account was created successfully.<br /><br />You may now continue to the <a href=\"login.php?do=login\">Login Page</a> and start playing ".$controlrow["gamename"]."!";
            }
        ...
        if ($controlrow["verifyemail"] == 1) { 
            $controlrow["verifytext"] = "<br />A verification code will be sent to the address above, and you will not be able to log in without first entering the code. Please be sure to enter your correct email address.";
        } else {
...
function lostpassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (sendpassemail($email,$newpass) == true) {
            ...
            display("There was an error sending your new password.<br /><br />Please check with the game administrator for more information.<br /><br />We apologize for the inconvenience.","Lost Password",false,false,false);
        }
        die();
    }
...
function changepassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (preg_match("/[^A-z0-9_\-]/", $newpass1)==1) { die("New password must be alphanumeric."); }
