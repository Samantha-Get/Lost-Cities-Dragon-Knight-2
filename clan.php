...
$query = doquery("SELECT * FROM {{table}} WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
while ($clanrow = mysql_fetch_array($query)) { 
$experience = $clanrow["experience"] - $userrow["level"]; }
$updatequery = doquery("UPDATE {{table}} SET experience='$experience' WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
$updateuserquery = doquery("UPDATE {{table}} SET clanid='0', memberstatus='0' WHERE id='".$userrow["name"]."' LIMIT 1", "users"); }
...
$query = doquery("INSERT INTO {{table}} SET id='', name='".$_POST['name']."', logo='".$_POST['logo']."', owner='".$userrow["name"]."'", "clans");
...
$query2 = doquery("UPDATE {{table}} SET clanid='".$userrow["name"]."', clanleader='1', memberstatus='7' WHERE id='".$userrow["name"]."' LIMIT 1", "users"); } }
else { 
...
while ($row = mysql_fetch_array($query)) { 
$clanquery = doquery("SELECT*FROM {{table}} WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
while ($clanrow = mysql_fetch_array($clanquery)) {

$k = $row["level"] * 2;
$newexperience = $clanrow["experience"] - $k;
$experienceupdate = doquery("UPDATE {{table}} SET experience='$newexperience' WHERE owner='".$userrow["name"]."' LIMIT 1", "clans"); } }
...
$usersupdate = doquery("UPDATE {{table}} SET clanid='".$userrow["name"]."' WHERE id='".$userrow["name"]."'", "users");//member accepted
$clanquery = doquery("SELECT*FROM {{table}} WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
while ($clanrow = mysql_fetch_array($clanquery)) { 
$k = $userrow["experience"] * 2;
$newexperience1 = $clanrow["experience"] + $k; //new xp
$update = doquery("UPDATE {{table}} SET experience='$newexperience1' WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
...
else { doquery("INSERT INTO {{table}} SET id='', clanid='$id', experience='".$userrow["level"]."', id='".$userrow["name"]."', username='".$userrow["charname"]."'", "users");
doquery("UPDATE {{table}} set usemine='1' WHERE id='".$userrow["name"]."'", "users");
...
$clanquery = doquery("SELECT logo FROM {{table}} WHERE owner='".$userrow["name"]."' LIMIT 1", "clans");
while ($clanrow = mysql_fetch_array($clanquery)) {
...
$page .= " <table width=\"80%\">";
$query = doquery("SELECT * FROM {{table}} WHERE clanid='".$userrow["name"]."' ORDER BY id DESC LIMIT 100", "users");
$rank = 1;
while ($row = mysql_fetch_array($query)) {
$clanquery = doquery("SELECT * FROM {{table}} WHERE id='".$row["userid"]."'", "users");
while ($clanrow = mysql_fetch_array($clanquery)) {
$page .= "<tr><td>Rank:   $rank</td><td>User Name:   <a href=\"index.php?do=onlinechar:".$row["userid"]."\" class=\"myButton2\">".$row["usersname"]."</a></td><td>Level:   ".$clanrow["level"]."<td width=\"100%\"><a href=\"index.php?do=yes:".$row["id"]."\" class=\"myButton2\">[Yes]</a></td><td width=\"100%\"><a href=\"index.php?do=no:".$row["id"]."\" class=\"myButton2\">[No]</a></td></tr>\n";
$rank++; } }
...
doquery("UPDATE {{table}} SET name='".$_POST['clanname']."' WHERE owner='".$userrow["name"]."' LIMIT 1", "clans"); }
...
doquery("UPDATE {{table}} SET logo='".$_POST['clanlogo']."' WHERE owner='".$userrow["name"]."' LIMIT 1", "clans"); }
