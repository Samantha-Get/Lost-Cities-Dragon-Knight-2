...
if (!isset($questrow)) {
    $questinfoquery = doquery("SELECT * from {{table}} WHERE id = '".$userrow["currentquestid"]."' LIMIT 1", "quests");
    $questrow = mysql_fetch_array($questinfoquery);
}
...
function quest() { // One big long function that determines the outcome of the quest.
    ...
    if ($questrow["quest_type"] == '0')
    {
	   ...
	   if ($userrow["experience"] + $exp < 101202303) { $newexp = $userrow["experience"] + $exp; $warnexp = ""; } else { $newexp = $userrow["experience"]; $exp = 0; $warnexp = "You have maxed out your experience points."; }
    	   if ($userrow["gold"] + $gold < 101202303) { $newgold = $userrow["gold"] + $gold; $warngold = ""; } else { $newgold = $userrow["gold"]; $gold = 0; $warngold = "You have maxed out your gold."; }
	   ...
	   $levelup = "false";
		...
		if ($levelup == "true") {
			$page = "<div align=\"center\"><table width=\"100%\"><tr><td><center><h3 class=\"title\">".$questrow["name"]." Completed<h3></center></td></tr></table><table width=\"100%\"><tr><td align=\"center\"><h3>" .$questrow["name"]. "</h3></td></tr><tr><td>" . nl2br($questrow["end_text"])."</td></tr></table><br /><br /><blockquote>Congratulations. You have completed the following quest: <br />".$questrow["name"]."<br /><br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold <br /><br />You have gained a level!<br /><br />You gain ".$levelrow[$userrow["charclass"]."_hp"]." hit points.<br />You gain ".$levelrow[$userrow["charclass"]."_mp"]." magic points.<br />You gain ".$levelrow[$userrow["charclass"]."_tp"]." travel points.<br />You gain ".$levelrow[$userrow["charclass"]."_strength"]." strength.<br />You gain ".$levelrow[$userrow["charclass"]."_dexterity"]." dexterity.<br />$spelltext</blockquote><br /><br />"; 
	  	} else {
			$page = "<table width=\"100%\"><tr><td><h3>" .$questrow["name"]. "</h3></td></tr><tr><td><blockquote>" . nl2br($questrow["end_text"])."</td></tr></table><br /><br /><blockquote>Congratulations. You have completed the following quest: <br />".$questrow["name"]."<br /><br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold</blockquote><br /><br />"; 
	  	}

	  	if ($questrow["drop_id"] != 0)
	  	{
			$dropcode = "dropcode='".$questrow["drop_id"]."',";
			$page .= "<blockquote>You have earned a reward for completing this quest. <a href=\"index.php?do=questdrop\" class=\"myButton2\">Click here</a><br /><br />to reveal and equip the item, or you may also move on and continue<br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a></blockquote>.";
	  	} else {
			$dropcode = "";
			$page .= "<font color=\"#804000\">You can now continue.</font><br><br><a href=\"index.php\" class=\"myButton2\">Exploring</a>."; 
	  	}
    	   }
    	   $updatequestquery = doquery("UPDATE {{table}} SET status='1' where quest_id='".$userrow["currentquestid"]."' AND user_id='".$userrow["id"]."' LIMIT 1", "questprogress");
    ...
    if ($userrow["currentfight"] == 1 && $questrow["quest_type"] == '1') {       
        ...
        $monsterquery = doquery("SELECT * FROM {{table}} WHERE id = '".$questrow["monster_id"]."' LIMIT 1", "monsters");
    ...
    if (isset($_POST["run"])) {
        ...
        unset($questrow);
        $updatequery = doquery("UPDATE {{table}} SET currentaction='Exploring', currentquestid='0' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
    ...
    // Do fight stuff.
        ...
        if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.
            ...
            if ($userrow["currenthp"] <= 0) {
                $newgold = ceil($userrow["gold"]/2);
                $newhp = ceil($userrow["maxhp"]/4);
                $updatequery = doquery("UPDATE {{table}} SET currenthp='$newhp',currentaction='In Town',currentmonster='0',currentmonsterhp='0',currentmonstersleep='0',currentmonsterimmune='0',currentfight='0',latitude='0',longitude='0',gold='$newgold',currentquestid='0' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
                $playerisdead = 1;
            }
        }
...
END;
    $updatequery = doquery("UPDATE {{table}} SET currentaction='Quest Event',currenthp='$newhp',currentmp='$newmp',currentfight='$newfight',currentmonster='$newmonster',currentmonsterhp='$newmonsterhp',currentmonstersleep='$newmonstersleep',currentmonsterimmune='$newmonsterimmune',currentuberdamage='$newuberdamage',currentuberdefense='$newuberdefense' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
} else {
    $pagearray["command"] = "<blockquote><blockquote>You have died.<br /><br />As a consequence, you've lost half of your gold. However, you have been given back a portion of your hit points to continue your journey.<br /><br />You may now continue back to the <br /><br /><a href=\"index.php\" class=\"myButton2\">Town Square</a><br /><br /> and we hope you fair better next time.</blockquote></blockquote>";
}
...
function questvictory() {
    ...
    if ($userrow["level"] < 300) {
        ...
        if ($levelup == "true") {
		$page = "<blockquote><blockquote>Congratulations. You have defeated the ".$monsterrow["name"].",<br />and you have completed the following quest:<br />".$questrow["name"]."<br /><br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold <br /><br />You have gained a level!<br /><br />You gain ".$levelrow[$userrow["charclass"]."_hp"]." hit points.<br />You gain ".$levelrow[$userrow["charclass"]."_mp"]." magic points.<br />You gain ".$levelrow[$userrow["charclass"]."_tp"]." travel points.<br />You gain ".$levelrow[$userrow["charclass"]."_strength"]." strength.<br />You gain ".$levelrow[$userrow["charclass"]."_dexterity"]." dexterity.<br />$spelltext<br /></blockquote></blockquote><br />"; 
	  } else {
		$page = "<blockquote><blockquote>Congratulations. You have defeated the ".$monsterrow["name"].",<br />and you have completed the following quest:<br />".$questrow["name"]."<br /><br />You gain $exp experience. $warnexp <br />You gain $gold gold. $warngold</blockquote></blockquote><br /><br /><br />"; 
	  }

	  if ($questrow["drop_id"] != 0)
	  {
		$dropcode = "dropcode='".$questrow["drop_id"]."',";
		$page .= "<blockquote><blockquote>You have earned a reward for completing this quest. <a href=\"index.php?do=questdrop\">Click here</a> to reveal and equip the item, or you may also move on and continue <a href=\"index.php\" class=\"myButton2\">Exploring</a></blockquote></blockquote>.";
	  } else {
		$dropcode = "";
		$page .= "<blockquote><blockquote><font color=\"#804000\">You can now continue.</font><br /><br /><a href=\"index.php\" class=\"myButton2\">Exploring</a>.</blockquote></blockquote>"; 
	  }
    }
    $updatequestquery = doquery("UPDATE {{table}} SET status='1' where quest_id='".$userrow["currentquestid"]."' AND user_id='".$userrow["id"]."' LIMIT 1", "questprogress");
    $updatequery = doquery("UPDATE {{table}} SET currentaction='Exploring',level='$newlevel',maxhp='$newhp',maxmp='$newmp',maxtp='$newtp',strength='$newstrength',dexterity='$newdexterity',attackpower='$newattack',defensepower='$newdefense', $newspell currentfight='0',currentmonster='0',currentmonsterhp='0',currentmonstersleep='0',currentmonsterimmune='0',currentuberdamage='0',currentuberdefense='0',$dropcode experience='$newexp',gold='$newgold',currentquestid='0' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
...
function questdrop() {
    ...
    if (isset($_POST["submit"])) {
        ...
        $page = "<table><tr><td align=\"center\"><h3 class=\"title\">Quest Information [root/quest.php]</h3></td></tr></table>
		<br><br><blockquote><blockquote>You have equipped this item, when you slide it into one of the many slots inside your Clothing. <font color=\"#804000\">You can now continue.</font><br><br><a href=\"index.php\" class=\"myButton2\">Exploring</a></blockquote></blockquote>";
    ...
    $page = "<blockquote>You have been rewarded the following item: ".$droprow["name"]."<br /><br />";
    ...
    $page .= "<br /><blockquote><blockquote>Select an inventory slot from the list below to equip this item. If the inventory slot is already full, the old item will be discarded.";
    $page .= "<form action=\"index.php?do=drop\" method=\"post\">
<select name=\"slot\"><option value=\"0\">Choose One</option>
...
</select> <input type=\"submit\" name=\"submit\" value=\"Submit\" /></form></blockquote></blockquote>";
    $page .= "<blockquote><blockquote>You may also choose to just continue <br /><br />a href=\"index.php\" class=\"myButton2\">Exploring</a><br /><br />and give up this item.</blockquote></blockquote>";
...
function dead() {
    ...
    $page = "<blockquote><blockquote>You have died.<br /><br />As a consequence, you've lost half of your gold. However, you have been given back a portion of your hit points to continue your journey.<br /><br />You may now continue back to the<br /><br /><a href=\"index.php\" class=\"myButton2\">Town Square</a><br /><br />and we hope you fair better next time. Remember, on your fifth death your body will be sent to the graveyard for its ever lasting final sleep.</blockquote></blockquote>";
